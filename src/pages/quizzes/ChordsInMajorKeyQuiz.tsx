// src/pages/quizzes/ChordsInMajorKeyQuiz.tsx
...
type QuestionMode = 'key-to-chord' | 'chord-to-key' | 'both';
type QuestionType = 'key-to-chord' | 'chord-to-key';
...
const DEGREES: { numeral: string; quality: DegreeQuality }[] = [
...
const QUIZ_LENGTHS = [10, 20, 40];

// Replace 'b' flat accidental with ♭ symbol for display
function formatLabel(raw: string): string {
  return raw.replace(/([A-G])b/g, '$1♭');
}

function getChordId(note: string, quality: DegreeQuality): string {
  if (quality === 'major') return note;
  if (quality === 'minor') return `${note}m`;
  return `${note}°`;
}

function getChordOptions(degreeIndex: number): Option[] {
  return ALL_KEYS.map(key => {
    const note = majorKeyData[key].notes[degreeIndex];
    const id = getChordId(note, DEGREES[degreeIndex].quality);
    return { id, label: formatLabel(id) };
  });
}
...
function makeQuestion(key: MajorScaleKey, degreeIndex: number, type: QuestionType): Question {
  const degree = DEGREES[degreeIndex];
  const note = majorKeyData[key].notes[degreeIndex];
  const chordId = getChordId(note, degree.quality);
  const chordLabel = formatLabel(chordId);

  if (type === 'key-to-chord') {
    return {
      type,
      key,
      degreeIndex,
      prompt: `What is the ${degree.numeral} chord in ${formatLabel(key)} major?`,
      correctAnswerId: chordId,
      options: getChordOptions(degreeIndex),
    };
  } else {
    return {
      type,
      key,
      degreeIndex,
      prompt: `${chordLabel} is the ${degree.numeral} chord in what major key?`,
      correctAnswerId: key,
      options: KEY_OPTIONS,
    };
  }
...
function buildQuestions(mode: QuestionMode, length: number): Question[] {
  const types: QuestionType[] = mode === 'both' ? ['key-to-chord', 'chord-to-key'] : [mode];
  const pool: Question[] = [];
  for (const type of types) {
    for (const key of ALL_KEYS) {
      for (let i = 0; i < 7; i++) {
        pool.push(makeQuestion(key, i, type));
      }
    }
  }
  ...
  if (phase === 'idle') {
    return (
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Chords in the Major Key</h1>
          <p className="text-gray-600">
            Practice identifying diatonic chords by Roman numeral in all 13 major keys.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 space-y-6">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([
                { value: 'key-to-chord', label: 'Key + Numeral → Chord' },
                { value: 'chord-to-key', label: 'Chord + Numeral → Key' },
          ...
          <div className={`p-3 rounded-lg mb-4 text-sm font-medium ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
            {isCorrect
              ...
              : `✗ Incorrect — the answer is ${
                  formatLabel(
                    currentQuestion.options.find(o => o.id === currentQuestion.correctAnswerId)?.label ?? currentQuestion.correctAnswerId
                  )
                }`
            }
          </div>
...
export default ChordsInMajorKeyQuiz;
